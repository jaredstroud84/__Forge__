...
const INK = "#0B1220";
const PANEL = "#121B2E";
const AMBER = "#CB5903"; // logo accent (copper/orange)
const CYAN = "#5FE3C9";
const TEXT = "#E9EEF7";
const MUTED = "#8FA0BC";
...
export default function BuildWithCode() {
  ...
  const [capstoneReadme, setCapstoneReadme] = useState("");
  const [capstoneDeployGuide, setCapstoneDeployGuide] = useState("");
  const [capstoneInterviewPitch, setCapstoneInterviewPitch] = useState("");
  ...
  async function buildCapstone() {
    ...
    try {
      const solutionsList = roadmap.levels
        ...
        .join("\n\n");
      const system = `You are assembling a beginner's finished project from the individual code pieces they wrote across a course, and packaging it like a professional software deliverable. Respond with ONLY raw JSON, no markdown fences: {"finalCode": "one complete, clean, working JavaScript file combining all their pieces into the final tool, with clear comments", "walkthrough": "a thorough section-by-section explanation (6-10 short paragraphs) of exactly how the finished tool works end to end, written so the student can explain it to someone else with full confidence — no hand-waving, no skipped steps", "conceptMap": "a bullet-style plain text list of every major coding concept they practiced, each followed by one sentence on where it appears in the finished tool", "readme": "a clean, professional README in plain text (markdown-style headers OK) with: a project title, one-paragraph description, a Features list, a How To Run section (plain-language steps for a total beginner, no assumed tools beyond a browser or Node), and a Tech Concepts Used list", "deployGuide": "a short, concrete, beginner-friendly guide (numbered steps) for putting this tool live on the free internet so others can use it — cover the simplest realistic free-hosting path for a small JS/HTML tool like this (e.g. a static host), written so someone who has never deployed anything before can follow it", "interviewPitch": "a polished 3-4 sentence paragraph, written in first person, that the student can paste directly into a resume bullet, LinkedIn post, or say out loud in a job interview, describing what they built and which real skills it demonstrates — confident, specific, zero fluff"}`;
      const prompt = `Final tool: ${roadmap.toolName} — ${roadmap.toolDescription}\n\nHere are the pieces the student wrote across each step:\n${solutionsList}\n\nAssemble these into one clean final tool, write the full walkthrough, the concept map, the README, the deploy guide, and the interview pitch.`;
      const text = await askClaude(prompt, system);
      const parsed = parseJSON(text);
      ...
      setCapstoneReadme(parsed.readme || "");
      setCapstoneDeployGuide(parsed.deployGuide || "");
      setCapstoneInterviewPitch(parsed.interviewPitch || "");
      saveFinishedTool({
...
<body>
  ...
  function downloadReadme() {
    ...
    const parts = [
      ...
      capstoneInterviewPitch ? "\n\n---\nYOUR INTERVIEW / RESUME / LINKEDIN PITCH\n" + capstoneInterviewPitch : "",
      "\n\n---\nBuilt with Forge — " + SITE_URL,
    ];
  ...
  function downloadCertificate() {
    ...
    const conceptCount = roadmap.levels ? roadmap.levels.length : 0;
    const html = `<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8" />
<meta name="viewport" content="width=device-width, initial-scale=1.0" />
<title>Certificate — ${roadmap.toolName || "My Tool"}</title>
<style>
  @import url('https://fonts.googleapis.com/css2?family=Space+Grotesk:wght@600;800&family=Inter:wght@400;600;700&display=swap');
  body { margin:0; min-height:100vh; display:flex; align-items:center; justify-content:center; background:linear-gradient(180deg,#0B1220 0%,#0E1830 100%); font-family:'Inter',sans-serif; }
  .cert { width:820px; max-width:92vw; background:#121B2E; border:1px solid rgba(203,89,3,0.35); border-radius:26px; padding:56px 48px; box-shadow:0 30px 80px rgba(0,0,0,0.45); text-align:center; color:#E9EEF7; }
  .brand { font-family:'Space Grotesk',sans-serif; font-weight:800; font-size:18px; letter-spacing:0.5px; color:#8FA0BC; }
  .brand span{color:#CB5903;}
  .kicker { margin-top:26px; font-size:12.5px; letter-spacing:2px; color:#5FE3C9; font-weight:700; }
  h1 { font-family:'Space Grotesk',sans-serif; font-size:34px; margin:18px 0 6px; letter-spacing:-0.5px; }
  .tool { font-family:'Space Grotesk',sans-serif; font-size:26px; font-weight:700; color:#CB5903; margin:14px 0 20px; }
  .desc { color:#8FA0BC; font-size:15px; max-width:560px; margin:0 auto 30px; line-height:1.6; }
  .stats { display:flex; justify-content:center; gap:40px; margin:26px 0 34px; flex-wrap:wrap; }
  .stat b { display:block; font-family:'Space Grotesk',sans-serif; font-size:22px; color:#E9EEF7; }
  .stat span { font-size:12px; color:#8FA0BC; letter-spacing:0.5px; }
  .date { font-size:13px; color:#8FA0BC; margin-top:10px; }
  .seal { margin-top:26px; display:inline-block; padding:8px 18px; border-radius:100px; border:1px solid rgba(203,89,3,0.4); color:#CB5903; font-size:12.5px; font-weight:700; letter-spacing:0.5px; }
</style>
</head>
<body>
  <div class="cert">
    ...
    <h1>This certifies that a real, working tool was built from scratch.</h1>
    <div class="tool">${roadmap.toolName || "My Tool"}</div>
    <div class="desc">${roadmap.toolDescription || ""}</div>
    <div class="stats">
      <div class="stat"><b>${conceptCount}</b><span>CONCEPTS MASTERED</span></div>
      <div class="stat"><b>100%</b><span>CODE WRITTEN &amp; EXPLAINED BY YOU</span></div>
      <div class="stat"><b>1</b><span>REAL TOOL, YOURS FOREVER</span></div>
    </div>
    <div class="seal">VERIFIED BUILD — NOT A TEMPLATE</div>
    <div class="date">Completed ${dateStr} · ${SITE_URL}</div>
  </div>
</body>
</html>`;
    const blob = new Blob([html], { type: "text/html" });
    ...
    const a = document.createElement("a");
    a.href = url;
    a.download = name + "-certificate.html";
    a.click();
    URL.revokeObjectURL(url);
  }
            ...
            {EXAMPLE_TOOLS.map((ex) => (
              <button
                ...
                style={{
                  ...
                  background: goal === ex.goal ? "rgba(203,89,3,0.1)" : "rgba(255,255,255,0.03)",
              ...
              <div style={{ display: "flex", flexDirection: "column", alignItems: "center", width: 28 }}>
                <div
                  style={{
                    ...
                    height: 28,
                    borderRadius: "50%",
                    background: completed[i] ? CYAN : sequenceUnlocked ? "rgba(203,89,3,0.2)" : "rgba(255,255,255,0.06)",
      ...
      <Shell>
        <Panel>
          ...
          <div style={{ color: MUTED, fontSize: 14, lineHeight: 1.6, marginBottom: 16 }}>
            The first {FREE_LEVELS} steps are free. Unlocking finishes <strong style={{ color: TEXT }}>{roadmap.toolName}</strong> and gives you everything below.
          </div>
          <ul style={{ listStyle: "none", margin: "0 0 20px", padding: 0 }}>
            {[
              "Your finished, working tool — downloadable code + HTML runner",
              "Full walkthrough of exactly how it works, section by section",
              "Concept map of every skill you practiced, tied to your code",
              "README + a free deploy guide to put it live on the internet",
              "A ready-to-use resume / LinkedIn / interview pitch about this build",
              "A downloadable completion certificate",
              "Unlimited mentor Q&A for every step",
              "You own everything forever — no subscription, no expiration",
            ].map((line, i) => (
              <li key={i} style={{ fontSize: 13.5, padding: "6px 0", display: "flex", gap: 10, alignItems: "flex-start", color: TEXT }}>
                <span style={{ color: CYAN, fontWeight: 800, flexShrink: 0 }}>✓</span>
                {line}
              </li>
            ))}
          </ul>
          <div
            style={{
              background: "rgba(203,89,3,0.08)",
              border: "1px solid rgba(203,89,3,0.3)",
              borderRadius: 16,
              padding: "20px",
              textAlign: "center",
              marginBottom: 20,
          ...
          >
            ...
            <div style={{ fontSize: 13, color: MUTED }}>
              one time — every item above, forever, no subscription
            </div>
          </div>
          <a
    ...
    return (
      <Shell>
        ...
        {capstoneCode && (
          <Panel>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
              ...
              <Btn variant="secondary" onClick={downloadReadme}>⬇ Download README + deploy guide</Btn>
              <Btn variant="secondary" onClick={downloadCertificate}>⬇ Download certificate</Btn>
            </div>
        ...
        {capstoneInterviewPitch && (
          <Panel>
            <div style={{ fontSize: 13, fontWeight: 700, color: CYAN, marginBottom: 10, letterSpacing: 0.5 }}>
              YOUR RESUME / LINKEDIN / INTERVIEW PITCH
            </div>
            <div style={{ fontSize: 13.5, lineHeight: 1.7, color: TEXT, whiteSpace: "pre-wrap" }}>
              {capstoneInterviewPitch}
            </div>
        ...
        <div style={{ fontSize: 12.5, color: MUTED, textAlign: "center", marginBottom: 16 }}>
          Working tool. README + deploy guide. Full walkthrough. Concept map. Interview pitch. Certificate.
          One flat $4.99, yours forever.
        </div>
